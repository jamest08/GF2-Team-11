...
def parser():
    ...
    file_list=["example1.txt", "example1_with_syntax_errors.txt", "example5_rubbish.txt", "example4.txt"]
    parser_list = []
...
def test_error_type(parser):
    "Test parser is returning correct errors by checking error messages are as expected."
    assert parser[0].scanner.error_message_list == []
    assert parser[1].scanner.error_message_list == ["Invalid symbol for start of line.","The following inputs are floating: ['G1.I1']"]
    assert "Expected 0 or 1 for switch state" and "Number of inputs must be integer in range(1, 17)" and "Unexpected dot. Can only specify port for DTYPE." in parser[3].scanner.error_message_list 
...
def test_connections(parser):
    ...
    [SW1_ID, SW2_ID, G1_ID,G2_ID, I1, I2] = names.lookup(["SW1", "SW2", "G1", "G2",
                                                     "I1","I2"])
